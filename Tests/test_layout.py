...
from Engine.layout import (
    ...
    book_cut_key, set_correction, stale_corrections, set_insert_text, split_insert, merge_insert,
)
...
def test_reczny_czas_nadpisuje_obie_sasiednie_plansze(layout, tokens):
    ...
    set_cut_time(layout, book_cut_key(granica), 123.456)
    boards = boards_from_layout(layout, tokens, _fake_times(tokens))
...
def test_reczny_czas_na_nieistniejacej_granicy_jest_odrzucany(layout):
    with pytest.raises(LayoutError, match="nie stoi żadna granica"):
        set_cut_time(layout, book_cut_key(layout.breaks[0] + 1), 10.0)


def test_czas_glowy_i_ogona_jest_dozwolony(layout, tokens):
    set_cut_time(layout, book_cut_key(0), 1.5)
    set_cut_time(layout, book_cut_key(layout.token_count), 999.0)
    boards = boards_from_layout(layout, tokens, _fake_times(tokens))
...
def test_scalenie_kasuje_czas_znikajacej_granicy(layout, tokens):
    granica = layout.breaks[0]
    set_cut_time(layout, book_cut_key(granica), 55.0)
    merge_boards(layout, 0)
    assert book_cut_key(granica) not in layout.overrides
...
def test_zapis_i_odczyt_zachowuja_podzial(tmp_path, layout):
    set_cut_time(layout, book_cut_key(layout.breaks[0]), 42.5)
...
def test_klucze_overrides_przezywaja_json(tmp_path, layout):
    set_cut_time(layout, book_cut_key(layout.breaks[0]), 7.25)
    store = LayoutStore(str(tmp_path))
    store.save(layout)
    with open(store.path(layout.chapter_num), encoding="utf-8") as f:
        surowy = json.load(f)
    assert list(surowy["overrides"]) == [book_cut_key(layout.breaks[0])]
    assert store.load(layout.chapter_num).overrides == {book_cut_key(layout.breaks[0]): 7.25}
...
def test_granice_da_sie_oznaczyc_jako_przejrzane(layout):
    set_reviewed(layout, book_cut_key(layout.breaks[0]))
    assert layout.reviewed == [book_cut_key(layout.breaks[0])]
    set_reviewed(layout, book_cut_key(layout.breaks[0]), False)
...
def test_scalenie_kasuje_znacznik_znikajacej_granicy(layout):
    granica = layout.breaks[0]
    set_reviewed(layout, book_cut_key(granica))
    merge_boards(layout, 0)
    assert book_cut_key(granica) not in layout.reviewed
...
def test_znacznik_na_nieistniejacej_granicy_jest_odrzucany(layout):
    with pytest.raises(LayoutError, match="nie stoi żadna granica"):
        set_reviewed(layout, book_cut_key(layout.breaks[0] + 1))
